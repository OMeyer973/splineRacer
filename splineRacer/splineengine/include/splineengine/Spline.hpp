...
class Spline {
    ...
    public:
        ...
        glm::vec3 const point(const float t);

        /// \brief returns the transforma matrix for the camera at the given point
        glm::mat4 const camMatrix(const glm::vec3& sPoint);
    ...
    protected:
        ...
        int  const loopInt(int i, int loopSize);
        ...
        glm::vec3 const getCatmullRomPosition(
            const float t,
            const glm::vec3& p0,
            const glm::vec3& p1,
            const glm::vec3& p2,
